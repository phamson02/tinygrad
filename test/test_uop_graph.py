...
class TestUOpGraph(unittest.TestCase):
  ...
  def test_noop_vectorize_fold(self):
    d0 = UOp(Ops.DEFINE_MEM, dtypes.float.ptr(), arg=0)
  ...
  def test_gep_vec_fold(self):
    d0 = UOp(Ops.DEFINE_MEM, dtypes.float.ptr(), (), 0)
    d1 = UOp(Ops.DEFINE_MEM, dtypes.float.ptr(), (), 1)
    d2 = UOp(Ops.DEFINE_MEM, dtypes.float.ptr(), (), 2)
    idx = UOp.const(dtypes.int, 0)
  ...
  def test_cast_alu_fold(self):
    d0 = UOp(Ops.DEFINE_MEM, dtypes.bool.ptr(), arg=0)
    d1 = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(), arg=1)
    idx = UOp.const(dtypes.int, 0)
  ...
  def test_bitcast_to_same_dtype_fold(self):
    for dt in dtypes.ints + dtypes.floats + (dtypes.bool,):
      d0 = UOp(Ops.DEFINE_MEM, dt.ptr(), arg=0)
  ...
  def test_in_out_of_bounds_access(self):
    with Context(IGNORE_OOB=0):
      glbl0 = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(16), (), 0)
  ...
  def test_in_bounds_access_gated_local(self):
    with Context(IGNORE_OOB=0):
      # Define buffers
      gbuf = UOp(Ops.DEFINE_MEM, dtypes.uint.ptr(400), (), 0)
      sbuf = UOp(Ops.DEFINE_MEM, dtypes.uint.ptr(8, local=True), (), "temp0")
  ...
  def test_in_out_of_bounds_access_index_load(self):
    with Context(IGNORE_OOB=0):
      glbl0 = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(16), (), 0)
      glbl1 = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(64), (), 0)
      gidx0 = UOp(Ops.SPECIAL, dtype=dtypes.int, arg=("gidx0", 42))
  ...
  def test_fold_gated_load(self):
    glbl0 = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(), (), 0)
    glbl1 = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(), (), 1)
    glbl2 = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(), (), 2)
    idx = UOp.const(dtypes.int, 0)
  ...
  def test_fold_gated_load_local(self):
    glbl0 = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(), (), 0)
    smem = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(size=18, local=True), (), "temp")
  ...
  def test_fold_gated_store(self):
    glbl = UOp(Ops.DEFINE_MEM, dtypes.int.ptr(), (), 0)
...
class TestIFUOps(unittest.TestCase):
  def test_create_ifs(self):
    gbuf = UOp(Ops.DEFINE_MEM, dtypes.float.ptr(), (), 0)
    sbuf = UOp(Ops.DEFINE_MEM, dtypes.float.ptr(size=4, local=True), (), "smem")
  ...
  def test_expand_ifs_one_gate(self):
    gbuf = UOp(Ops.DEFINE_MEM, dtypes.float.ptr(), (), 0)
    sbuf = UOp(Ops.DEFINE_MEM, dtypes.float.ptr(size=16, local=True), (), "smem")
