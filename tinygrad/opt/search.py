...
from tinygrad.dtype import ImageDType, PtrDType, is_ptr
...
def bufs_from_lin(lin:Kernel, allocate:bool=True) -> list[Buffer]:
  ...
  for x in lin.bufs:
    if x.src[0].base.op is Ops.DEFINE_MEM and is_ptr(x.src[0].base.dtype, local=False): bufsts[x.src[0].base.arg].append(x)
