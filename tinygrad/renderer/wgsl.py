from tinygrad.dtype import DType, PtrDType, dtypes, is_ptr
...
class WGSLRenderer(CStyleLanguage):
  ...
  string_rewrite = PatternMatcher([
    ...
    (UPat(Ops.DEFINE_MEM, name="x"), lambda ctx,x:
      f"var<workgroup> {ctx[x]}: array<{ctx.buf_map(x.dtype.base)},{x.dtype.size//(4//x.dtype.itemsize) if is_packed(x.dtype) and x.dtype.local else x.dtype.size}>;"),
