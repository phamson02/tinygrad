...
class ProgramSpec:
  ...
  def __post_init__(self):
    if not self._ran_post_init and self.uops is not None:
      ...
      for u in self.uops:
        ...
        if u.op is Ops.DEFINE_MEM: self.globals.append(u.arg)
        if u.op is Ops.STORE: self.outs.extend([x.arg for x in u.src[0].toposort() if x.op is Ops.DEFINE_MEM])
        if u.op is Ops.LOAD: self.ins.extend([x.arg for x in u.src[0].toposort() if x.op is Ops.DEFINE_MEM])
  ...
  def mem_estimate(self) -> sint:
    ...
    return sum(max(x.src[0].dtype.nbytes() for x in group)
      for _, group in itertools.groupby([x for x in self.ast.toposort() if x.op in {Ops.LOAD, Ops.STORE} and x.src[0].base.op is Ops.DEFINE_MEM],
