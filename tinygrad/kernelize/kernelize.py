...
merge_views = PatternMatcher([
  ...
  (UPat(GroupOp.All-{Ops.DEFINE_MEM}).view(name="view"),
...
add_buffer_ops = PatternMatcher([
  # LOAD
  (UPat(Ops.BUFFER, name="x"), lambda ctx,x: UOp.load(UOp(Ops.DEFINE_MEM, x.dtype.ptr(x.size), (), ctx.index(x)).view(x.st),)),
  ...
  (UPat(Ops.SINK, src=UPat(GroupOp.All-{Ops.STORE}), name="sink"), lambda ctx,sink:
   UOp.sink(*[UOp.store(UOp(Ops.DEFINE_MEM, (s:=x.base).dtype.ptr(ctx[i].size), (), i).view(s.st), s) for i,x in enumerate(sink.src)])),
  # passthrough ASSIGN
...
fix_kernel_ops = PatternMatcher([
  ...
  (UPat(GroupOp.All-{Ops.DEFINE_MEM, Ops.VIEW}, name="x"), lambda x: x.replace(dtype=x.dtype.base) if isinstance(x.dtype, ImageDType) else None),
